...
common_format = function(){
    ...
    new = scale_x_date(
            breaks = c(as.Date("1951-01-01"),
                       ...
                       as.Date("2020-01-01")), 
            date_label = "%Y", 
            limits= as.Date(c("1950-01-01","2022-07-01")),
            expand = c(0, 0)
        ) 
    out = c(out, list(new))
}
    ...
    fig = ggplot(data, aes(x = date)) +
        ...
        scale_x_date(
            breaks = c(as.Date("1946-01-01"),
                       ...
                       as.Date("2020-01-01")),
            date_label = "%Y",
            limits = as.Date(c("1945-01-01","2022-07-01")),
            expand = c(0, 0)
        ) +
        xlab("Year") +
        scale_y_continuous(
...
plot_pci_since_2012 = function(data, event=TRUE,abs=TRUE){
    common_format = function(){
        ...
        new = scale_x_date(
                breaks = seq.Date(as.Date("2012-01-01"),  as.Date("2021-07-01"),by="1 year"), 
                date_label = "%Y", 
                limits= as.Date(c("2011-10-01","2021-10-01")),
                expand = c(0, 0)
            ) 
        out = c(out, list(new))
    }
